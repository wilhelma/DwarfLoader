...
class ArchRule {
 protected:
  ...
  virtual std::unique_ptr<artifacts_t> execute(Artifact_t &archSet, const Context &ctxt) = 0;
  virtual std::unique_ptr<artifacts_t> append(Artifact_t &archSet, const Context &ctxt) = 0;

 public:
  ArchRule() = default;
  virtual ~ArchRule() = default;

  void apply(Artifact_t &archSet, const Context &ctxt)
  {
